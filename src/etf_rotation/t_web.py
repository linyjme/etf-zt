...
function renderNav(items){watchCount.textContent=items.length+' 项';watchList.innerHTML=items.map(item=>{const marketAt=item.timestamp?new Date(item.timestamp):null,fresh=item.status==='OK'&&Number.isFinite(marketAt&&marketAt.getTime())&&Date.now()-marketAt.getTime()<=STALE_AFTER_MS,opportunity=fresh&&(item.action==='BUY_REMINDER'||item.action==='SELL_REMINDER');return `<button type="button" class="watch-item${item.symbol===selectedSymbol?' active':''}${opportunity?' opportunity':''}" data-symbol="${esc(item.symbol)}" aria-current="${item.symbol===selectedSymbol?'true':'false'}"><span class="watch-item-main"><span class="watch-item-name">${esc(item.name)}</span><span class="watch-item-symbol">${esc(item.symbol)}</span></span><i class="watch-item-state${item.status==='MISSING_QUOTE'?' missing':''}${opportunity?' opportunity':''}"></i></button>`}).join('')||'<div class="empty">监控列表为空</div>'}
function render(data){latestData=data;const refreshedAt=new Date(),allItems=data.items||[],items=showMissing.checked?allItems:allItems.filter(item=>item.status!=='MISSING_QUOTE');errors.textContent=(data.errors||[]).join(' · ');if(!items.some(item=>item.symbol===selectedSymbol))selectedSymbol=items.length?items[0].symbol:null;renderNav(items);const item=items.find(current=>current.symbol===selectedSymbol);if(!item){detail.innerHTML=`<div class="card empty">${allItems.length?'无可显示行情，请开启“显示无行情”':'监控列表为空'}</div>`;staleBanner.classList.remove('visible');connectionStatus.classList.remove('stale');statusNode.textContent='实时监控中';dot.classList.add('live');dot.classList.remove('stale');refreshTimeNode.textContent='页面刷新时间：'+refreshedAt.toLocaleString();return}const marketAt=item.timestamp?new Date(item.timestamp):null,stale=item.status==='OK'&&(!Number.isFinite(marketAt.getTime())||refreshedAt.getTime()-marketAt.getTime()>STALE_AFTER_MS),cls=item.action==='SELL_REMINDER'?'up':item.action==='BUY_REMINDER'?'down':'wait',move=item.change_pct!=null&&item.change_pct>=0?'up':'down',missing=item.status==='MISSING_QUOTE',golden=!stale&&(item.action==='BUY_REMINDER'||item.action==='SELL_REMINDER'),goldenAlert=golden?`<div class="golden-alert ${item.action==='BUY_REMINDER'?'buy':'sell'}" role="alert"><strong>做T黄金窗口</strong><span>${item.action==='BUY_REMINDER'?'回补提醒':'减仓提醒'} · ${esc(item.label)}</span></div>`:'';const regimeClass=item.regime_state==='UPTREND'?'uptrend':item.regime_state==='DOWNTREND'?'downtrend':item.regime_state==='RANGE'?'range':'uncertain';const regimeAlert=`<div class="regime-alert ${regimeClass}" role="status"><strong>${esc(item.regime_state==='UPTREND'?'上涨趋势日':item.regime_state==='DOWNTREND'?'下跌趋势日':item.regime_state==='RANGE'?'震荡日':'状态未知')}</strong><span> · ${esc(item.regime_label||'状态未确认，暂停做T')}</span></div>`;detail.innerHTML=`<article class="card" data-symbol="${esc(item.symbol)}"><div class="top"><div><div class="symbol">${esc(item.symbol)}</div><div class="name">${esc(item.name)}</div></div><div><div class="price">${num(item.price)}</div><div class="pct ${move}">${pct(item.change_pct)}</div></div></div><div class="signal ${cls}">${esc(item.status)} · ${esc(item.label)}</div>${regimeAlert}${goldenAlert}${missing?'<div class="empty">行情缺失，指标不可用</div>':chart(item)}<div class="meta"><div><span>黄线均价</span>${num(item.average_price)}</div><div><span>昨收零轴</span>${num(item.previous_close)}</div><div><span>白黄偏离</span>${item.white_yellow_deviation_grids==null?'—':num(item.white_yellow_deviation_grids)+'格'}</div><div><span>离昨收</span>${item.previous_close_distance_grids==null?'—':num(item.previous_close_distance_grids)+'格'}</div><div><span>快速上冲</span>${item.fast_rise_grids==null?'—':num(item.fast_rise_grids)+'格'}</div><div><span>格宽</span>${pct(item.grid_width_pct)}</div><div><span>上格</span>${num(item.upper_grid_price)}</div><div><span>下格</span>${num(item.lower_grid_price)}</div></div><section class="alerts"><h3>按日历史行情</h3><div id="daily-history" class="alert-list">${dailyHistoryCache.get(`${historyDate.value}|${item.symbol}`)||'<div class="hint">选择日期后查看当日分钟行情</div>'}</div></section><section class="alerts"><h3>提示追溯</h3><div id="alert-list" class="alert-list">${alertHistoryCache.get(item.symbol)||'<div class="hint">正在载入提示历史</div>'}</div></section><div class="time market-time${stale?' stale':''}">行情数据时间：${marketAt&&Number.isFinite(marketAt.getTime())?marketAt.toLocaleString():'—'}</div>${backtestSummary(item.symbol)}</article>`;refreshTimeNode.textContent='页面刷新时间：'+refreshedAt.toLocaleString();staleBanner.classList.toggle('visible',stale);connectionStatus.classList.toggle('stale',stale);statusNode.textContent=stale?'当前行情已过期':'实时监控中';dot.classList.toggle('live',!stale);dot.classList.toggle('stale',stale);loadAlerts();loadDailyHistory()}
