...
def _load_json_object(path: Path, label: str) -> dict[str, object]:
    try:
        ...
        raise ValueError(f"{label}读取失败: {error}") from error
...
def _require_exact_keys(
    ...
    if actual != expected:
        ...
        raise ValueError(f"{label}字段无效: missing={missing}, extra={extra}")
...
def load_watchlist(
    ...
    if type(payload["schema_version"]) is not int or payload["schema_version"] != 1:
        raise ValueError("波段监控列表schema_version必须是整数1")
    raw_items = payload["items"]
    if not isinstance(raw_items, list):
        raise ValueError("波段监控列表items必须是数组")
    ...
    for raw_item in raw_items:
        ...
        if (
            ...
            raise ValueError("波段监控代码必须是6位ASCII数字")
        enabled = raw_item["enabled"]
        if type(enabled) is not bool:
            raise ValueError(f"{symbol}.enabled必须是布尔值")
        if symbol in seen:
            raise ValueError(f"波段监控代码重复: {symbol}")
        if symbol not in metadata:
            raise ValueError(f"波段监控代码缺少ETF元数据: {symbol}")
        seen.add(symbol)
...
def load_strategy(path: Path) -> SwingStrategyConfig:
    ...
    if type(payload["schema_version"]) is not int or payload["schema_version"] != 1:
        raise ValueError("波段策略schema_version必须是整数1")
    if payload["strategy_version"] != "SWING_V1":
        raise ValueError("波段策略strategy_version必须是SWING_V1")
    ...
    if not values["short_ma_days"] < values["long_ma_days"] <= values["minimum_daily_bars"]:
        raise ValueError("必须满足short_ma_days < long_ma_days <= minimum_daily_bars")
    if values["entry_zone_atr_half_width"] > values["pullback_atr_distance"]:
        raise ValueError("entry_zone_atr_half_width不能超过pullback_atr_distance")
    if values["pullback_atr_distance"] >= values["anti_chase_atr_distance"]:
        raise ValueError("pullback_atr_distance必须小于anti_chase_atr_distance")
    if values["add_profit_r"] >= values["reduce_profit_r"]:
        raise ValueError("add_profit_r必须小于reduce_profit_r")
    if values["initial_stop_atr"] > values["trailing_stop_atr"]:
        raise ValueError("initial_stop_atr不能超过trailing_stop_atr")
    ...
    if values["max_symbol_weight"] > values["max_equity_weight"]:
        raise ValueError("max_symbol_weight不能超过max_equity_weight")
