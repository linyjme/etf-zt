...
function marketMinute(timestamp){const value=new Date(timestamp),minutes=value.getHours()*60+value.getMinutes();if(minutes<=690)return Math.max(0,minutes-570);return Math.min(240,120+Math.max(0,minutes-780))}
...
function chart(item){
  ...
  const pathValue=valueForPoint=>points.map((point,index)=>(index?'L':'M')+x(point).toFixed(1)+' '+y(valueForPoint(point)).toFixed(1)).join(' '),path=key=>pathValue(point=>point[key]);
