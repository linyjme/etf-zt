...
.t-account-summary{margin:10px 0;padding:10px 12px;border:1px solid var(--line);border-radius:9px;background:#081321;font-size:12px;overflow-wrap:anywhere}.t-account-summary-head{display:flex;justify-content:space-between;gap:12px;margin-bottom:5px}.t-account-summary-values{display:flex;flex-wrap:wrap;gap:5px 18px;color:var(--muted)}.t-account-summary-values b{color:var(--text);font-weight:500}.t-account-summary .hint{margin-top:4px}
...
const T_ACCOUNT_REFRESH_MS=60000,T_ACCOUNT_TIMEOUT_MS=5000;
const tAccountState={phase:'LOADING',payload:null,lastAttemptAt:null,pending:null,requestId:0};

function tAccountSummaryMarkup(symbol,state=tAccountState){
  const heading='<div class="t-account-summary-head"><strong>当前标的持仓</strong><a href="/portfolio">资产总览</a></div>';
  const message=text=>heading+`<div class="hint">${esc(text)}</div>`;
  if(state.phase==='LOADING')return message('正在加载持仓报告…');
  if(state.phase!=='READY')return message('持仓暂不可用');
  const payload=state.payload,view=payload&&payload.holdings_snapshot;
  if(!view||typeof view!=='object')return message('持仓暂不可用');
  if(view.status==='ABSENT')return message('未登记持仓报告，无法据此判断实际持仓');
  const snapshot=view.snapshot;
  if(view.status!=='SNAPSHOT_ONLY'||payload.status!=='SNAPSHOT_ONLY'||!snapshot||!Array.isArray(snapshot.positions))return message('持仓暂不可用');
  const matched=snapshot.positions.filter(position=>position&&position.symbol===symbol);
  if(!matched.length)return message('当前标的未登记持仓，无法据此判断实际持仓');
  if(matched.length!==1)return message('持仓暂不可用');
  const position=matched[0],amount=(value,digits=0)=>typeof value==='number'&&Number.isFinite(value)&&value>=0?value.toFixed(digits):'—';
  const sourceTime=typeof snapshot.positions_as_of==='string'&&snapshot.positions_as_of?snapshot.positions_as_of:'—';
  return heading+`<div class="t-account-summary-values"><span>报告份额：<b>${amount(position.shares)}</b></span><span>报告可卖：<b>${amount(position.sellable_shares)}</b></span><span>报告成本：<b>${amount(position.average_cost,3)}</b></span><span>来源时点（持仓截图）：<b>${esc(sourceTime)}</b></span></div><div class="hint">仅为已登记报告，不代表当前实际可卖；不改变做T配置或回测。</div>`;
}

function renderTAccountSummary(){
  const node=document.querySelector('#t-account-summary');
  if(!node||node.dataset.symbol!==selectedSymbol)return;
  node.innerHTML=tAccountSummaryMarkup(selectedSymbol,tAccountState);
}

function loadTAccountSummary(){
  const nowMs=Date.now();
  if(tAccountState.pending)return tAccountState.pending;
  if(tAccountState.lastAttemptAt!==null&&nowMs-tAccountState.lastAttemptAt<T_ACCOUNT_REFRESH_MS)return Promise.resolve();
  const requestId=++tAccountState.requestId,controller=new AbortController();
  tAccountState.lastAttemptAt=nowMs;tAccountState.phase='LOADING';tAccountState.payload=null;
  renderTAccountSummary();
  let timeout;
  const request=(async()=>{const response=await fetch('/api/portfolio',{cache:'no-store',signal:controller.signal});if(!response.ok)throw new Error('账户请求失败');return response.json()})();
  const deadline=new Promise((resolve,reject)=>{timeout=setTimeout(()=>{controller.abort();reject(new Error('账户请求超时'))},T_ACCOUNT_TIMEOUT_MS)});
  const pending=Promise.race([request,deadline]).then(payload=>{
    if(requestId!==tAccountState.requestId)return;
    tAccountState.phase='READY';tAccountState.payload=payload;renderTAccountSummary();
  }).catch(()=>{
    if(requestId!==tAccountState.requestId)return;
    tAccountState.phase='ERROR';tAccountState.payload=null;renderTAccountSummary();
  }).finally(()=>{clearTimeout(timeout);if(requestId===tAccountState.requestId)tAccountState.pending=null});
  tAccountState.pending=pending;
  return pending;
}
...
function render(data){
  ...
  detail.querySelector('.top')?.insertAdjacentHTML('afterend',`<section id="t-account-summary" class="t-account-summary" data-symbol="${esc(item.symbol)}" aria-label="当前标的持仓摘要">${tAccountSummaryMarkup(item.symbol,tAccountState)}</section>`);
...
loadTAccountSummary();setInterval(loadTAccountSummary,T_ACCOUNT_REFRESH_MS);
