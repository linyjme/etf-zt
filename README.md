...
这是一个本地运行的 ETF 监控器，提供三个一级入口：`/portfolio` 资产总览、`/` 分钟级做 T 观察、`/swing` 日线波段计划与回测。账户持仓统一在资产总览展示，两个策略页只显示当前标的的报告持仓摘要。三个页面共用一个服务进程；做 T 和波段的策略状态与 revision 仍相互隔离。系统不连接券商，不读取券商账号，也不会提交交易委托。
...
- `src/etf_rotation/portfolio_page.py`：系统级只读资产总览、报告持仓与来源说明。
...
打开 `/portfolio` 查看已录入的完整账户快照，包括报告总资产、可用现金、证券仓位、其他资产和持仓明细。金额明确标记为报告值而非实时估值；持仓截图与账户截图保留各自时点。来源说明可展开，未知值保持为“—”，零份可卖不表示未知。长期股票只记账，不进入 ETF 策略。

`GET /api/portfolio` 复用原有已发布账户读模型，与 `GET /api/swing/portfolio` 保持一致，不另存一份账户、不在请求中读取行情或刷新策略。接口只读、禁止缓存，页面不提供成交或策略激活按钮。原 `/` 和 `/swing` 地址不变；账户摘要读取失败不会中断做 T 行情显示。私人快照仍保留原路径，不因页面归属调整而搬迁或改写。
...
资产总览显示完整 `holdings_snapshot`，做 T 与波段页面只保留当前标的摘要：报告份额、报告可卖量、显示精度的成本及来源时间。新旧账户读接口兼容。长期股票仅记账、不进入 ETF 策略；其他资产不增加可用现金。缺失价格、可卖量、日期和止损保留未知，不以成本代替市价，不强行凑齐不同时间截图的金额。
...
脚本从自身位置推导项目根目录，在同一个隐藏 Python 进程中提供 `/portfolio`、`/` 和 `/swing`，并只写一个 PID。静态配置读取 `data/monitor` 与 `data/swing`，运行文件分别写入 `var/monitor` 与 `var/swing`，所有路径均以项目根目录为基准显式传入。
