...
这是一个以 ETF 日线波段监控为主的本地研究与执行辅助工具，核心入口是 `/swing`：用于查看完成日线、趋势与回调计划、指标观察、数据质量、影子策略、回测以及本地成交账本。`/` 保留为分钟级做 T 观察，`/pr` 提供估值旁路，`/notifications` 提供本地通知观察；这些页面共用一个服务进程，波段、做 T 与通知的状态和 revision 相互隔离。系统不连接券商，不读取券商账号，也不会提交交易委托。
